...
-- подойти со строчкой 54, почему b
...
myFoldr :: (a -> b -> b) -> b -> [a] -> b
myFoldr f init (x:xs) = x `f` (myFoldr f init xs)
myFoldr f init [] = init
...
myPartition p xs = foldl localF ([], []) xs
    where -- localF :: a -> ([a], [a]) -> ([a], [a])
         localF (xs, ys) z | p z = (z : xs, ys)
                            | otherwise = (xs, z : ys)
